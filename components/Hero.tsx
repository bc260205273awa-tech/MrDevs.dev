      ...
      <div className="relative max-w-6xl mx-auto px-6 pt-36 pb-24 w-full">
        ...
        <div className="hero-line-1 inline-flex items-center gap-2.5 px-4 py-2 border border-[rgba(0,180,216,0.2)] rounded-full mb-10"
          ...
          <span className="relative flex h-2 w-2">
            <span className="ping-slow absolute inline-flex h-full w-full rounded-full bg-[#00d4aa]" />
            <span className="relative inline-flex h-2 w-2 rounded-full bg-[#00d4aa]" />
          </span>
          <span className="font-manrope text-[11px] font-semibold text-[#00d4aa] tracking-widest uppercase">
            Available for new projects
          </span>
        </div>

        <div className="mb-8 max-w-4xl">
          <h1 className="hero-line-2 font-syne font-black text-[clamp(2.8rem,7vw,5.8rem)] leading-[1.0] tracking-tight">
            We Build
          </h1>
          <h1 className="hero-line-3 font-syne font-black text-[clamp(2.8rem,7vw,5.8rem)] leading-[1.0] tracking-tight gradient-text">
            Digital Products
          </h1>
          <h1 className="hero-line-4 font-syne font-black text-[clamp(2.8rem,7vw,5.8rem)] leading-[1.0] tracking-tight">
            That Drive Results.
          </h1>
        </div>

        <p className="hero-line-5 font-manrope text-[#8892a4] text-lg md:text-xl leading-relaxed max-w-xl mb-10">
        ...
        <div className="hero-line-5 flex flex-wrap gap-3 mb-20">
          <a href="#contact"
            className="btn-primary group inline-flex items-center gap-2.5 px-7 py-3.5 text-sm font-manrope rounded-[6px]">
            Start a Project
          ...
          <a href="https://wa.me/923218492868" target="_blank" rel="noopener noreferrer"
            className="inline-flex items-center gap-2.5 px-7 py-3.5 border font-manrope font-medium text-sm rounded-[6px] text-white transition-all duration-300 hover:border-[rgba(0,212,170,0.5)] hover:bg-[rgba(0,212,170,0.07)]"
            style={{ borderColor: "rgba(255,255,255,0.1)" }}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" className="text-[#00d4aa]">
        ...
        <div className="hero-line-5 flex flex-wrap gap-4 pt-8 border-t" style={{ borderColor: "rgba(0,180,216,0.12)" }}>
          {VALUE_PROPS.map(({ icon, label }) => (
            <div key={label} className="flex items-center gap-2.5 px-4 py-2.5 rounded-[6px] border"
              ...
              <span className="font-manrope text-xs font-semibold text-[#8892a4] tracking-wide uppercase">{label}</span>
            </div>
          ))}
        </div>
      </div>
