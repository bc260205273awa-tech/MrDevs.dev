...
export default function Footer() {
  return (
    <footer className="relative border-t py-14" style={{ borderColor: "rgba(0,180,216,0.1)" }}>
      ...
      <div className="max-w-6xl mx-auto px-6">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-10 mb-10">
          <div>
            <a href="/" className="flex items-center gap-3 mb-3">
              <div className="w-7 h-7 flex items-center justify-center rounded-[5px]"
              ...
              <span className="font-syne font-bold text-white tracking-tight text-[15px]">
                Mr<span style={{ color: "#00b4d8" }}>Devs</span>
              </span>
            </a>
            <p className="font-manrope text-[#4a5568] text-xs max-w-[220px] leading-relaxed">
          ...
          <nav className="flex gap-8">
            {NAV_LINKS.map(({ label, href }) => (
              <a key={label} href={href}
                className="font-manrope text-sm text-[#4a5568] hover:text-white transition-colors duration-200">{label}</a>
            ))}
          </nav>
          <div className="flex gap-2">
            {SOCIALS.map(({ label, title, href }) => (
              <a key={label} href={href} target="_blank" rel="noopener noreferrer" aria-label={title}
                ...
                <span className="font-syne font-bold text-[10px] text-[#4a5568] group-hover:text-white transition-colors duration-300">{label}</span>
              </a>
            ))}
          </div>
        </div>
        <div className="flex flex-col sm:flex-row items-center justify-between gap-2 pt-6 border-t"
          ...
          <p className="font-manrope text-[#2e3a4e] text-xs">© {new Date().getFullYear()} MrDevs. All rights reserved.</p>
          <p className="font-manrope text-xs" style={{ color: "#00b4d8", opacity: 0.5 }}>mrdevs.dev</p>
        </div>
      </div>
    </footer>
  );
