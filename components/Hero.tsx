...
export default function Hero() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      // Dynamically select navbar items (since we can't edit Navbar directly)
      const navItems = document.querySelectorAll('header > div > a.group, header nav a, header > div > a[href="#contact"], header > div > button');
      ...
      // INITIAL STATE: hide everything immediately to prevent flash
      gsap.set([navItems, badge, headlines, subtext, btns, stats, scrollIndicator], { opacity: 0 });
  ...
  return (
    <section ref={containerRef} className="relative min-h-screen flex flex-col justify-center overflow-hidden bg-[#0a1628]"
      ...
      <div className="relative max-w-6xl mx-auto px-6 pt-36 pb-24 w-full">
        ...
        <div className="gsap-badge inline-flex items-center gap-2.5 px-4 py-2 border border-[rgba(0,180,216,0.2)] rounded-full mb-10"
        ...
        <div className="mb-8 max-w-4xl">
          <h1 className="gsap-headline font-syne font-black text-[clamp(2.8rem,7vw,5.8rem)] leading-[1.0] tracking-tight text-white">
            We Build
          </h1>
          <h1 className="gsap-headline font-syne font-black text-[clamp(2.8rem,7vw,5.8rem)] leading-[1.0] tracking-tight">
        ...
        <p className="gsap-subtext font-manrope text-[rgba(255,255,255,0.65)] text-lg md:text-xl leading-relaxed max-w-xl mb-10">
        ...
        <div className="flex flex-wrap gap-3 mb-20">
          <a href="#contact"
            className="gsap-btn group inline-flex items-center gap-2.5 px-7 py-3.5 text-sm font-manrope rounded-[6px] bg-[#00b4d8] text-[#0a1628] hover:bg-[#00d4aa] transition-colors">
            Start a Project
          ...
          <a href="https://wa.me/923218492868" target="_blank" rel="noopener noreferrer"
            className="gsap-btn inline-flex items-center gap-2.5 px-7 py-3.5 border font-manrope font-medium text-sm rounded-[6px] text-[#00b4d8] bg-transparent transition-all duration-300 hover:bg-[rgba(0,180,216,0.1)]"
        ...
        <div className="flex flex-wrap gap-4 pt-8 border-t" style={{ borderColor: "rgba(0,180,216,0.12)" }}>
          {VALUE_PROPS.map(({ icon, label }) => (
            <div key={label} className="gsap-stat flex items-center gap-2.5 px-4 py-2.5 rounded-[6px] border"
      ...
      <div className="gsap-scroll absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 opacity-30 pointer-events-none">
